...
def train_lgb_model(self, DNase_h5, motif_h5, cell_line, chrom, TFname, label_file_train, label_file_test, knn_n, dir_out):
	print(DNase_h5)
	print(motif_h5)
	print(cell_line)
	print(chrom)
	...
	np.random.seed(2019)
	#used_col_for_train = np.concatenate((np.arange(0,48), np.arange(160,199)), axis=0)
	used_col_for_train = np.concatenate((np.arange(0,48), np.arange(160,199)), axis=0)
	kmeans = KMeans(n_clusters=knn_n, random_state=0).fit(np.concatenate((train_data[:,used_col_for_train], test_data[:,used_col_for_train]), axis=0))
	#kmeans = KMeans(n_clusters=knn_n, random_state=0).fit(train_data[:,used_col_for_train])
	#kmeans_c = kmeans.cluster_centers_
	#train_data_used = train_data[:,used_col_for_train]
	#kmeans_c = np.mean(train_data_used[kmeans.labels_==0,:], axis=0).reshape(1, train_data_used.shape[1])
	#for ii in range(1,knn_n):
	#	kmeans_c_ii = np.mean(train_data_used[kmeans.labels_==ii,:], axis=0).reshape(1, train_data_used.shape[1])
	#	kmeans_c = np.concatenate((kmeans_c, kmeans_c_ii), axis=0)

	print('kmeans_c.shape')
	#print(kmeans_c.shape)
	...
	#dist_c = get_dist(kmeans_c, test_data[:,used_col_for_train])
	...
	for kkk in range(0,knn_n):
		...
		print('random split')
		np.random.seed(2019)
		random_indices = np.random.choice(train_data1.shape[0], size=int(train_data1.shape[0]*0.1), replace=False)
		...
		print('lgb beta')
		print(train_data_lgb.get_label())
		print(ypred.shape)
		print(labels.shape)
		print(np.where(train_data_lgb.get_label() > 0)[0].shape[0])
		if np.where(train_data_lgb.get_label() > 0)[0].shape[0] > 0 :
			...
			print(ypred[used_id0_test].shape)
			print(gbm.predict(test_data)[used_id0_test].shape)
			print(np.zeros(np.sum(used_id0_test)).shape)
		else:
			print(ypred[used_id0_test].shape)
			print(np.zeros(np.sum(used_id0_test)).shape)
			ypred[used_id0_test] = np.zeros(np.sum(used_id0_test))
			labels = (df_temp[:,3]=='B')*1
		print('final')
		print(kkk)
		print(average_precision_score(labels0[used_id0_test], ypred[used_id0_test]))
