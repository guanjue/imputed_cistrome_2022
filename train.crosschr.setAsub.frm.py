...
def train_lgb_model(self, DNase_h5, DNase_h5_test, motif_h5, motif_h5_test, sigtype, cell_line, chrom, chrom_test, TFname, label_file_train_topN, label_file_train, label_file_test, dir_out, train_ct, test_ct):
	...
	print('read DNase')
	DNase_h5a = DNase_h5+chrom+'_all_cell_types.'+sigtype+'.ave.'+train_ct+'.'+test_ct+'.h5'
	hf = h5py.File(DNase_h5a, 'r')
	dnase_a = np.array(hf.get(chrom))
	dnase_a = dnase_a[:,:,0:24]+dnase_a[:,:,24:48]
	...
	train_chr = ['chr4', 'chr6', 'chr7', 'chr12', 'chr13']#, 'chr15', 'chr16', 'chr17', 'chr20']
	#train_chr = ['chr1', 'chr21', 'chr8']
	...
	for chrom in train_chr:
		...
		dnase_a_i = dnase_a_i[:,:,0:24] + dnase_a_i[:,:,24:48]
	...
	#train_data = a#np.concatenate((a, a), axis=1)[used_rows,:]
	del dnase_a
	del a
	...
	print('test motif')
	for chrom_test in ['chr12', 'chr4', 'chr6']:
		...
		DNase_h5_test1 = DNase_h5_test+chrom_test+'_all_cell_types.'+sigtype+'.ave.'+train_ct+'.'+test_ct+'.h5'
		hf = h5py.File(DNase_h5_test1, 'r')
		dnase_at = np.array(hf.get(chrom_test))
		hf.close()
		dnase_at = dnase_at[np.where(dnase_samples=='test')[0][0],:,:]
		dnase_at = dnase_at[:,0:24]+dnase_at[:,24:48]
		test_data = np.concatenate((dnase_at, at), axis=1)
