...
def train_lgb_model(self, DNase_h5, motif_h5, cell_line, chrom, TFname, label_file_train, label_file_test, dir_out):
	...
	train_id = np.array(pd.read_csv('HepG2_Epithelium_Liver.train.id.list.txt', sep="\t", header=None))

        ### get where are the train & test ids 
	train_id_pos = np.intersect1d(dnase_samples, train_id, return_indices=True)[1]

	print('get train data')
	print(dnase_samples)
	print(train_id)
	print(np.intersect1d(dnase_samples, train_id, return_indices=True))
	print(train_id_pos)

	train_data = np.concatenate((dnase_a[train_id_pos[0],:,:], a), axis=1)
	print('get weight & label for training')
	weight = (df_temp[:,3]!='A')*1
	labels = (df_temp[:,3]=='B')*1
	for k in range(1, len(train_id_pos.shape)):
		train_data_k = np.concatenate((dnase_a[train_id_pos[k],:,:], a), axis=1)
		train_data = np.concatenate((train_data, train_data_k), axis=0)
		weight = np.concatenate((weight, (df_temp[:,3]!='A')*1), axis=0)
		labels = np.concatenate((labels, (df_temp[:,3]=='B')*1), axis=0)
	...
	test_id = np.array(pd.read_csv('MCF-7_Epithelium_Breast.test.id.list.txt', sep="\t", header=None))
	test_id_pos = np.intersect1d(dnase_samples, test_id, return_indices=True)[1]
	print(dnase_a[test_id_pos,:,:].shape)
	test_data = np.concatenate((np.mean(dnase_a[test_id_pos,:,:], axis=0), a), axis=1)
