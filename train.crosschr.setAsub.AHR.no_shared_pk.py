...
def train_lgb_model(self, DNase_h5, DNase_h5_test, motif_h5, motif_h5_test, sigtype, cell_line, chrom, chrom_test, TFname, label_file_train_topN, label_file_train, label_file_test, dir_out, train_ct, test_ct):
	print(DNase_h5)
	print(motif_h5)
	print(cell_line)
	print(chrom)
	...
	common_pk_binary = np.array(pd.read_csv(DNase_h5+chrom+'_all_cell_types.'+sigtype+'.ave'+'.log2FC.txt', sep="\t", header=None))
	common_pk_binary = common_pk_binary.reshape(common_pk_binary.shape[0],1)
	...
	for chrom in train_chr:
		...
		DNase_h5a = DNase_h5+chrom+'_all_cell_types.'+sigtype+'.ave'+'.h5'
		common_pk_binary_i = np.array(pd.read_csv(DNase_h5+chrom+'_all_cell_types.'+sigtype+'.ave'+'.log2FC.txt', sep="\t", header=None))
		common_pk_binary_i = common_pk_binary_i.reshape(common_pk_binary_i.shape[0],1)
		common_pk_binary = np.concatenate((common_pk_binary, common_pk_binary_i), axis=0)
		hf = h5py.File(DNase_h5a, 'r')
	...
	common_pk_binary_train = common_pk_binary[used_rows,:]
	...
	print(common_pk_binary_train.shape)
	print(np.sum(labels))
	...
	common_pk_binary_1 = np.delete(common_pk_binary_train, random_indices, 0)
	train_data_2 = train_data[random_indices,:]
	common_pk_binary_2 = common_pk_binary_train[random_indices,:]
	del train_data
	...
	dif = train_data_1[:,0:24]+train_data_1[:,24:48]
	print(common_pk_binary_1.shape)
	print(train_data_1.shape)
	print(labels_1.shape)
	print(weight_1.shape)
	used_train_row = ((np.absolute(common_pk_binary_1[:,0])>2) + (labels_1==0) ) >0
	print('left bins')
	print(used_train_row.shape[0])
	print(np.sum(used_train_row))
	print(np.sum(used_train_row)/used_train_row.shape[0])
	train_data_lgb = lgb.Dataset(train_data_1[used_train_row,:], label=labels_1[used_train_row], weight=weight_1[used_train_row])
	used_test_row = ((np.absolute(common_pk_binary_2[:,0])>2) + (labels_2==0)) >0
	test_data_lgb = lgb.Dataset(train_data_2[used_test_row,:], label=labels_2[used_test_row], weight=weight_2[used_test_row])
	...
	for i in range(1,23):
		...
		outputname = dir_out+TFname+'.'+train_ct+'.'+test_ct+'.'+str(cell_line)+'.'+chrom+'.'+chrom_test+'.lgb.pred.txt'
