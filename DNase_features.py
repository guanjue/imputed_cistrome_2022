...
class DNaseFeatures(object):
    ...
    # @staticmethod
    def combine_dnase_features_h5_for_all_cell_types(self, chrom, infile_path, outfile_path, train_id_file, test_id_file):
        ...
        with h5py.File(
            ...
            temp = []
            print(cell_types)
            train_id = np.array(pd.read_csv(train_id_file, sep="\t", header=None))[:,0]
            test_id = np.array(pd.read_csv(test_id_file, sep="\t", header=None))[:,0]
            print(train_id)
            cell_types = np.concatenate((train_id, test_id), axis=0)
            print(cell_types)
            for cell_line in cell_types:
                ...
                if not self.CA_cluster_mode:
                    ...
                    for cell_line_id in self.df_cluster_info_file[self.df_cluster_info_file['cluster']==int(cell_line)]['sample'].values:
                        h5_file_in = "%s/DNASE_bam_5_mer_variable_bp_all_samples_lightGBM_%s.%s.h5" % (infile_path, chrom,
                                                                                             str(cell_line_id))
