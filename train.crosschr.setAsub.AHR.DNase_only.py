...
def train_lgb_model(self, DNase_h5, DNase_h5_test, motif_h5, motif_h5_test, sigtype, cell_line, chrom, chrom_test, TFname, label_file_train_topN, label_file_train, label_file_test, dir_out, train_ct, test_ct):
	...
	train_data = dnase_a[np.where(dnase_samples=='train')[0][0],:,:][used_rows,:]
	del dnase_a
	del a
	...
	for i in range(1,23):
		...
		test_data = dnase_at[np.where(dnase_samples=='train')[0][0],:,:]
		ypred = gbm.predict(test_data)
		outputname = dir_out+TFname+'.'+train_ct+'.'+test_ct+'.'+str(cell_line)+'.'+chrom+'.'+chrom_test+'.lgb.DNase_only.pred.txt'
