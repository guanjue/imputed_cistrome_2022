...
def train_lgb_model(self, DNase_h5, motif_h5, cell_line, chrom, TFname, label_file_train, label_file_test, knn_n, dir_out):
	...
	ypred = test_data[:,0]
	np.random.seed(2019)
	alldat = np.concatenate((train_data[:,np.concatenate((np.arange(0,24), np.arange(24,train_data.shape[1])), axis=0)], test_data[:,np.concatenate((np.arange(0,24), np.arange(24,train_data.shape[1])), axis=0)]), axis=0)
	kmeans = KMeans(n_clusters=knn_n, random_state=0).fit(alldat)
	#kmeans = KMeans(n_clusters=knn_n, random_state=0).fit(train_data[:,np.concatenate((np.arange(0,48), np.arange(160,199)), axis=0)])
