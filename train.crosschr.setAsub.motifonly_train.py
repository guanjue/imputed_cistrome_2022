...
def train_lgb_model(self, DNase_h5, DNase_h5_test, motif_h5, motif_h5_test, sigtype, cell_line, chrom, chrom_test, TFname, label_file_train_topN, label_file_train, label_file_test, dir_out, train_ct, test_ct):
	...
	train_chr = ['chr3']#, 'chr15', 'chr16', 'chr17', 'chr20']
	...
	for i in [5]:
		...
		test_data = np.concatenate((dnase_at[np.where(dnase_samples=='train')[0][0],:,:], at), axis=1)
		...
		auprc_0 = average_precision_score(labels, ypred)
		print('Overall AUPRC')
		print(auprc_0)
		
		outputname = dir_out+TFname+'.'+train_ct+'.'+test_ct+'.'+str(cell_line)+'.'+chrom+'.'+chrom_test+'.lgb.predtrain.txt'
		for j in range(0,48):
			test_data_j = np.copy(test_data)
			np.random.shuffle(test_data_j[:,j])
			ypred = gbm.predict(test_data_j)
			outputname = dir_out+TFname+'.'+train_ct+'.'+test_ct+'.'+str(cell_line)+'.'+chrom+'.'+chrom_test+'.lgb.predtrain.txt'
			...
			df_temp = d[d[:,0] == chrom_test, :]
			labels = (df_temp[:,3]=='B')*1
			auprc_i = average_precision_score(labels, ypred)
			print(auprc_i)
			auprc_vec.append(auprc_i)
		
		
		for j in range(0,24):
			test_data_j = np.copy(test_data)
			np.random.shuffle(test_data_j[:,j])
			np.random.shuffle(test_data_j[:,j+24])
			ypred = gbm.predict(test_data_j)
			ypred = ypred.reshape(ypred.shape[0],1)
			df_temp = d[d[:,0] == chrom_test, :]
			labels = (df_temp[:,3]=='B')*1
		...
		#for l in range(0,10):
		for j in range(48, test_data.shape[1], 40):
			test_data_j = np.copy(test_data)
			for k in range(0,40):
				np.random.shuffle(test_data_j[:,j+k])
			ypred = gbm.predict(test_data_j)
			df_temp = d[d[:,0] == chrom_test, :]
			labels = (df_temp[:,3]=='B')*1
			auprc_i = average_precision_score(labels, ypred)
		...
		auprc_vec = np.array(auprc_vec)
		np.savetxt(outputname+'shuffle.txt', auprc_vec, fmt='%10.2f', delimiter='\t')
