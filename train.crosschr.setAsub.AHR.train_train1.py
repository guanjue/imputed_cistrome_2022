...
def train_lgb_model(self, DNase_h5, DNase_h5_test, motif_h5, motif_h5_test, sigtype, cell_line, chrom, chrom_test, TFname, label_file_train_topN, label_file_train, label_file_test, dir_out, train_ct, test_ct):
	...
	for j in range(0,48):
		train_data[:,j] = train_data[:,j]/np.max(train_data[:,j])
	...
	for i in range(1,23):
		...
		for j in range(0,48):
			test_data[:,j] = test_data[:,j]/np.max(test_data[:,j])
		#test_data = at#np.concatenate((dnase_at[np.where(dnase_samples=='test')[0][0],:,:], at), axis=1)
		ypred = gbm.predict(test_data)
		outputname = dir_out+TFname+'.'+train_ct+'.'+test_ct+'.'+str(cell_line)+'.'+chrom+'.'+chrom_test+'.lgb.pred1.txt'
