...
mkdir -p 'lgbmodel/'$TF
mkdir -p 'lgbmodel/'$TF'/'$train_cl_ct_tis'_'$test_cl_ct_tis
...
/usr/bin/time --verbose python $scripts_dir/train.crosschr.setAsub.human2mouse.py train_lgb_model --chrom=$chrom_j --chrom_test=$chrom_j_test --TFname=$TF \
