...
def train_lgb_model(self, DNase_h5, motif_h5, cell_line, chrom, TFname, label_file_train, label_file_test, km_n, dir_out):
	print(DNase_h5)
	print(motif_h5)
	print(cell_line)
	print(chrom)
	...
	alldat = np.concatenate((train_data[:,np.concatenate((np.arange(48,50), np.arange(50,train_data.shape[1])), axis=0)], test_data[:,np.concatenate((np.arange(48,50), np.arange(50,test_data.shape[1])), axis=0)]), axis=0)
	...
	alldat = np.concatenate((train_data[:,np.concatenate((np.arange(48,50), np.arange(50,test_data.shape[1])), axis=0)], test_data[:,np.concatenate((np.arange(48,50), np.arange(50,test_data.shape[1])), axis=0)]), axis=0)
	...
	df_temp0 = d[d[:,0] == chrom, :]
	labels0 = (df_temp0[:,3]=='B')*1
