	...
	def SQTKNNN(self, input_h5file, output_h5file, DNase_ave_f_DR_file, DNase_ave_f_file, DNase_ave_r_DR_file, DNase_ave_r_file, CP_id_file, chrom, fdr_thresh = 0.1, q_thresh = 0.95, r_thresh = 0.6, bin_size = 25, knn_n = 1, sn = 1):
		...
		for i in range(0, score_new.shape[0]):
			...
			for j in range(0,2):
				...
				score_mat25_SQT_KNNa = KNN_norm(CP_id_new, score_mat25a_test, score_mat25_SQTa.reshape(score_mat25a_test.shape[0], 1), score_mat25_ave_spline_DR, q_thresh, r_thresh)
		...
		for i in range(0, score_new.shape[0]):
			...
			for j in range(2,4):
				...
				score_mat25_SQT_KNNa = np.copy(score_mat25_SQTa)
				print('get KNN norm matrix')
				score_mat25_SQT_KNNa = KNN_norm(CP_id_new, score_mat25a_test[:,0].reshape(score_mat25a_test.shape[0], 1), score_mat25_SQTa.reshape(score_mat25a_test.shape[0], 1), score_mat25_ave_spline_DR, q_thresh, r_thresh)
