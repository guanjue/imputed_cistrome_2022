...
def train_lgb_model(self, DNase_h5, othersig, motif_h5, cell_line, chrom, TFname, label_file_train, label_file_test, dir_out):
	print(DNase_h5)
	print(motif_h5)
	print(cell_line)
	print(chrom)
	...
	otherinfo = np.array(pd.read_csv(othersig, sep="\t", header=None)).reshape(a.shape[0],1)
	otherinfo[otherinfo>0]=1
	a = np.concatenate((a, otherinfo), axis=1)
