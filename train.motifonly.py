...
def train_lgb_model(self, DNase_h5, motif_h5, cell_line, chrom, TFname, label_file_train, dir_out):
	### functions & params
	...
	train_data = a#np.concatenate((dnase_a[np.where(dnase_samples==str(cell_line))[0][0],:,:], a), axis=1)
	...
	print('lgb beta')
	beta = - np.log10(2 * train_data.shape[0]/np.where(train_data_lgb.get_label() > 0)[0].shape[0] - 1)
	...
	test_data = a#np.concatenate((dnase_a[np.where(dnase_samples=='test')[0][0],:,:], a), axis=1)
